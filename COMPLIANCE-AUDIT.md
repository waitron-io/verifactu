...
| `FechaFinVeriFactu`, `Incidencia`, `RefRequerimiento`, and `FinRequerimiento`                | Parsing enforces the XSD date shape, `S`/`N` enums, and 18-code-point reference maximum; serialization also requires nonblank reference content and the published real-date/year window | Focused parser/serializer tests and exact 18/19-code-point schema probes                                                        | XML Schema accepts an empty `TextMax18Type` and Unicode decimal digits; serialization is deliberately stricter. AEAT's clock and issued requirement remain authoritative |
