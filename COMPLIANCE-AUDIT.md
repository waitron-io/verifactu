...
| `PeriodoImputacion`                                      | `ConsultaFiltro` requires a four-digit year and one of `01`–`12`. The fake now filters on `FechaOperacion`, falling back to `FechaExpedicionFactura`, and a red/green integration test distinguishes June from July                                                                            | AEAT alone proves how corrected historical data is assigned and ordered in its store                                               |
